...
const tool = {
	...
	//
	countHandScore(handResult: HandOutcome | undefined,
		shouldConsiderPre: boolean = false): number{
		if (!handResult){
			return 0
		}
		const result = this._countScoreByWait(handResult, shouldConsiderPre)
		return result
	},
	...
	_countScoreByWait(handOutcome: HandOutcome,
		shouldConsiderPre: boolean = false): number{
		const bhand = handOutcome.bancoHand
		const phand = handOutcome.puntoHand
		const cards: Card[] = [
